...
class Mutasi_model extends CI_Model
{
	...
	function updateNeraca($kode){
		...
		foreach($jurnal as $j){
			...
			if($j['debit'] != 0){
				...
				$this->db->set('saldo', 'saldo+'.$j['debit'], FALSE);
				$this->db->update('akun');
			}else{
				$this->db->where('kd_akun', $kd_akun);
				$this->db->set('saldo', 'saldo-'.$j['kredit'], FALSE);
				$this->db->update('akun');
			}
		}
	}
	...
	{
		$hasil = $this->db->query("SELECT * FROM akun WHERE kd_akun LIKE 'A01.01.%.00' AND kd_akun NOT LIKE kd_akun='A01.01.02.%' AND kd_akun NOT LIKE kd_akun='A01.01.02.%' ")->result();
		return $hasil;
	}
