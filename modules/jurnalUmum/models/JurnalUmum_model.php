...
class JurnalUmum_model extends CI_Model
{
	...
	function getData()
	{
		$this->db->select('*');
		$this->db->from('jurnal');
		$this->db->like('kd_transaksi', 'L-', 'after'); 
		$hasil = $this->db->get();
		return $hasil->result();
	}
	function setData()
	{
		...
		$this->kd_transaksi = 'L-'.$this->kd_data;
	...
	{
		$hasil = $this->db->query("SELECT * FROM akun WHERE kd_akun LIKE 'A02.01.%' AND NOT kd_akun='A02.01.00.00' AND NOT kd_akun='A02.01.01.00' AND NOT kd_akun='A02.01.02.00' AND NOT kd_akun='A02.01.02.00' ")->result();
		return $hasil;
	}
	...
	{
		$hasil = $this->db->get_where("jurnal", array('kd_transaksi' => 'L-'.$kode))->result();
		return $hasil;
	}
	function updateData()
	{
