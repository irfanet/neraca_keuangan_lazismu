    ...
    <div class="modal-content">
      ...
      <form id="form_add" data-parsley-validate class="form-horizontal form-label-left">
        <div class="modal-body">
          <input type="hidden" id="id_3" name="id_3">
          <div class="row">
          ...
          <div class="row">
            <div class="form-group">
              ...
              <div class="col-md-6 col-sm-6 col-xs-6">
                <input type="text" id="kd_3" name="kd_3" required class="form-control col-md-7 col-xs-12">
              </div>
            </div>
          </div>
          <br>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    document.getElementById("id_1").onchange = function () {
		$("#id_2").select2('val', '');
	}
    ...
    function tampil_data() {
      $.ajax({
        type: 'ajax',
        url: '<?= base_url() ?>setting/getSegmen3',
        async: false,
        dataType: 'json',
        success: function(data) {
          ...
          for (i = 0; i < data.length; i++) {
            html += '<tr>' +
              ...
              '<a href="javascript:;" class="btn btn-info btn-xs item_edit" data="' + data[i].id_3 + '">Edit</a>' + ' ' +
              '<a href="javascript:;" class="btn btn-danger btn-xs item_hapus" data="' + data[i].id_3 + '">Hapus</a>' +
              '</td>' +
              '</tr>';
          }
          $('#show_data').html(html);
    ...
    $('#show_data').on('click', '.item_edit', function() {
      ...
      $.ajax({
        type: "GET",
        url: "<?= base_url() ?>setting/getDataByKode3",
        dataType: "JSON",
        data: {
          id: id
        },
        success: function(data) {
          $.each(data, function(id_3, kd_3, nama, id_2) {
            $('#modal_add').modal('show');
            $('[name="show_in_add"]').hide();
            $('[name="show_in_edit"]').show();
            $('#id_3').val(data.id_3);
            $('#id_2').val(data.id_2);
            $('#kd_3').val(data.kd_3).attr('readonly', true);
            $('#nama').val(data.nama);
            $('#modal_add').on('shown.bs.modal', function() {
              $('#nama').focus()
            });
    ...
    $('#btn_simpan').on('click', function() {
      if (kondisi == "tambah") {
        $.ajax({
          type: "POST",
          url: "<?= base_url() ?>setting/setSegmen3",
          dataType: "JSON",
          data: {
            id_2: $('#id_2').val(),
            kd_3: $('#kd_3').val(),
      ...
      //EDIT DATA
        ...
        $.ajax({
          type: "POST",
          url: "<?= base_url() ?>setting/updateSegmen3",
          dataType: "JSON",
          data: {
            id_2: $('#id_2').val(),
            kd_3: $('#kd_3').val(),
    ...
    $('#btn_hapus').on('click', function() {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url() ?>setting/deleteSegmen3",
        dataType: "JSON",
        data: {
          kode: kode
        },
