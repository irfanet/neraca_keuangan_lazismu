    ...
    <div class="modal-content">
      ...
      <form id="form_add" data-parsley-validate class="form-horizontal form-label-left">
        <div class="modal-body">
          <input type="hidden" id="id_4" name="id_4">
          <div class="row">
          ...
          <div class="row">
            <div class="form-group">
                <label class="control-label col-md-4 col-sm-4 col-xs-4" for="kode">Kode Segmen 2 <span class="required">*</span>
                </label>
                <div class="col-md-6 col-sm-6 col-xs-6">
                    <select type="text" class="form-control" id="id_2" name="id_2" required>
                    <!-- //isian -->
                    </select>
                </div>
            </div>
          ...
          <div class="row">
            <div class="form-group">
                <label class="control-label col-md-4 col-sm-4 col-xs-4" for="kode">Kode Segmen 3 <span class="required">*</span>
                </label>
                <div class="col-md-6 col-sm-6 col-xs-6">
                    <select type="text" class="form-control" id="id_3" name="id_3" required>
                    <!-- //isian -->
                    </select>
                </div>
            </div>
          ...
          <div class="row">
            <div class="form-group">
              ...
              <div class="col-md-6 col-sm-6 col-xs-6">
                <input type="text" id="kd_4" name="kd_4" required class="form-control col-md-7 col-xs-12">
              </div>
            </div>
          </div>
          <br>
  ...
  $(document).ready(function() {
    ...
    function getSegmen2(){
        $('#id_1').change(function(){
            var id_1=$(this).val();
            $.ajax({
                url : "<?php echo base_url();?>setting/getSegmenById1",
                method : "POST",
                data : {id_1: id_1},
                async : false,
                dataType : 'json',
                success: function(data){
                    ...
                    for(i=0; i<data.length; i++){
                        html += '<option value="' + data[i].id_2 + '">'+data[i].kd_2+' - ' +data[i].nama+ ' - ' +data[i].id_2+'</option>';
                    }
                    $('#id_2').html(html);
    ...
    function getSegmen3(){
        $('#id_2').change(function(){
            var id_2=$(this).val();
            $.ajax({
                url : "<?php echo base_url();?>setting/getSegmenById2",
                method : "POST",
                data : {id_2: id_2},
                async : false,
                dataType : 'json',
                success: function(data){
                    ...
                    var i;
                    for(i=0; i<data.length; i++){
                        html += '<option value="' + data[i].id_3+ '">'+data[i].kd_3+' - ' +data[i].nama+' - ' +data[i].id_3+'</option>';
                    }
                    $('#id_3').html(html);
    ...
    function tampil_data() {
      $.ajax({
        type: 'ajax',
        url: '<?= base_url() ?>setting/getSegmen4',
        async: false,
        dataType: 'json',
        success: function(data) {
          ...
          for (i = 0; i < data.length; i++) {
            html += '<tr>' +
              ...
              '<a href="javascript:;" class="btn btn-info btn-xs item_edit" data="' + data[i].id_4 + '">Edit</a>' + ' ' +
              '<a href="javascript:;" class="btn btn-danger btn-xs item_hapus" data="' + data[i].id_4 + '">Hapus</a>' +
              '</td>' +
              '</tr>';
          }
          $('#show_data').html(html);
    ...
    $('#show_data').on('click', '.item_edit', function() {
      ...
      $.ajax({
        type: "GET",
        url: "<?= base_url() ?>setting/getDataByKode4",
        dataType: "JSON",
        data: {
          id: id
        },
        success: function(data) {
          $.each(data, function(id_4, kd_4, nama, id_3) {
            $('#modal_add').modal('show');
            $('[name="show_in_add"]').hide();
            $('[name="show_in_edit"]').show();
            $('#id_4').val(data.id_4);
            $('#id_3').val(data.id_3);
            $('#kd_4').val(data.kd_4).attr('readonly', true);
            $('#nama').val(data.nama);
            $('#modal_add').on('shown.bs.modal', function() {
              $('#nama').focus()
            });
    ...
    $('#btn_simpan').on('click', function() {
      if (kondisi == "tambah") {
        $.ajax({
          type: "POST",
          url: "<?= base_url() ?>setting/setSegmen4",
          dataType: "JSON",
          data: {
            id_3: $('#id_3').val(),
            kd_4: $('#kd_4').val(),
      ...
      //EDIT DATA
        ...
        $.ajax({
          type: "POST",
          url: "<?= base_url() ?>setting/updateSegmen4",
          dataType: "JSON",
          data: {
            id_2: $('#id_2').val(),
            kd_3: $('#kd_3').val(),
    ...
    $('#btn_hapus').on('click', function() {
      ...
      $.ajax({
        type: "POST",
        url: "<?= base_url() ?>setting/deleteSegmen4",
        dataType: "JSON",
        data: {
          kode: kode
        },
